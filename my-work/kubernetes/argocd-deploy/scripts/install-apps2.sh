...
detect_namespaces() {
  ...
  echo ""
  log_info "Holding the script for (30s) some resources to be created"
  sleep 30
  
  echo ""
  log_info "Detecting apps namespaces (dev, stage, prod)"
  
  ENVIRONMENTS=$(kubectl get ns -o jsonpath='{.items[*].metadata.name}' \
  | tr ' ' '\n' \
  | grep -E '^(dev|stage|prod)$')
  
  echo ""
  log_info "Namespaces detected ${ENVIRONMENTS}"
